'''
    Intro to Binary files
        -> Binary files are files that can contain everything that's stored on your computer's disk drives.
        That also includes text files.

        -> Python automatically decodes the binary data that it reads from the files, and splits it up at the
        line breaks.
